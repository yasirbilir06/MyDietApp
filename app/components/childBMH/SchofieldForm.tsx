...
import {
  ...
  calculateBMI,
} from '../../utils/childBMHCalculations';

export default function SchofieldForm() {
  ...
  const handleCalculate = () => {
    ...
    const bmh = calculateSchofield(ageNum, weightNum, heightNum, gender); // ✅ cm giriliyor, fonksiyonda m'ye çevriliyor
    const bmi = calculateBMI(weightNum, heightNum); // ✅ aynı şekilde
    const ideal = calculateIdealWeight(ageNum);
    const total = bmh * faNum;
    ...
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.card}>
        ...
        <View style={styles.genderContainer}>
          <Text>Erkek</Text>
          <Switch value={isMale} onValueChange={setIsMale}
        ...
        <Text style={[styles.result, { color: 'green' }]}>BMH: {bmh.toFixed(0)} kcal</Text>
        <Text style={[styles.result, { color: 'green' }]}>Gereksinim: {total.toFixed(0)} kcal</Text>
      </View>
    </ScrollView>
  );
}
